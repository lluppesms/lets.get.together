...
### Account Identity Foundation update (2026-08-27)

The application account is `UserId`; `User` no longer stores a provider subject or email address. `UserIdentity` stores a provider-constrained Entra, Google, or Facebook identity using the unique `(Provider, Issuer, Subject)` tuple. `UserEmailAlias` stores separately verified aliases; its normalized address is unique only when verified. `IUserRepository` resolves users through provider-qualified identities and persists initial or subsequently linked identities and aliases. `ICurrentUserResolver` is the authenticated page boundary: it derives the provider only from the validated authentication scheme (`OpenIdConnect`, `Google`, or `Facebook`), requires `iss` plus `NameIdentifier`, and resolves the corresponding identity tuple without using email.
...
The Entra token-validation callback preserves the validated token issuer and normalizes validated `sub`/`oid` subject claims to `ClaimTypes.NameIdentifier` in the application cookie alongside the protected provider marker, so `CurrentUserResolver` can resolve the provider-qualified identity after redirect. The circles entry path provisions a first-time signed-in user from that validated provider identity, allowing immediate circle creation; invitations remain available for joining existing circles. The circles empty state offers circle creation only after account resolution succeeds and otherwise directs the user to sign out and retry authentication.

Circle creation assigns the creator the `Owner` membership role. Members added through invitations or direct member operations receive the `Member` role.

Identity resolution and first-time provisioning use structured `ILogger` entries. The `GetTogether.Web` category is enabled at `Debug`; resolution failures include the authenticated display name when supplied by the provider, authentication type, available claim types, provider, and presence flags, while provisioning logs include provider, issuer, lookup result, and full database exceptions without logging subject values, tokens, or secrets.
...
`IAccountIdentityService` in `src/web/Data/Services/` owns email-bound onboarding and account identity operations. Secure invitations include recipient and normalized recipient email fields; `EmailVerificationToken` persists only SHA-256 hashes with a single-use timestamp and expiry. `CompleteOnboardingAsync` verifies the recipient-bound invite, verification token, and provider-qualified identity before atomically creating the user, verified primary alias, linked identity, and circle membership. The service has no email-to-identity matching path. Provider link/unlink operations require a caller-supplied recent-authentication proof and prohibit removal of a user's last identity. Alias operations add a pending alias, verify it, designate one verified primary alias, or remove it. `IVerificationEmailSender` uses `SendGridVerificationEmailSender` only when both `SendGrid:ApiKey` and `SendGrid:FromEmail` are configured; otherwise it explicitly uses `NoOpVerificationEmailSender`. The DACPAC now defines `Meetings.EmailVerificationToken` and enforces one primary alias per user.

### Account identity UI update (2026-08-27)
...
Phase 1 authentication UI surfaces now include `/login`, `/signup`, and `/signup-callback` under `src/web/Website/Pages/`. The shared `LoginDisplay` links anonymous users to `/login`; Microsoft Entra uses the existing Microsoft Identity UI when `AzureAD:TenantId` is configured. Google and Facebook authenticate only when their respective ID and secret configuration pairs are both present, exposing `/login/google` and `/login/facebook` challenges. The external ticket handlers ensure the stable provider issuer and `NameIdentifier` claims required by `CurrentUserResolver`. Signup currently validates invitation-code shape and carries the code to the callback route; server-side validation and redemption remain a contract boundary.
